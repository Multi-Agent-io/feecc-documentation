...
Роутер [`employee`](гх линк на файл) отвечает за информацию и взаимодейтсвие с **сотрудником**: логин, логаут, информация об
этапе сборки. Эндпоинты:

- `post /info`: вся информация о сотруднике на текущем РМИ

- `post /log-in`: логин сотрудника в систему на текущем РМИ

- `post /log-out`: логаут сотрудника
...
- `get /new`: создается новое изделие. Аргументом является рабочая схема, которая является разной для разных типов продуктов.

- `post /{unit_internal_id}/info`: получение всей информации об изделиии: процессы, схемы, статус, история, компоненты итд.

Стоит отметить, что частью ссылки является `unit_internal_id` - 10-значный десятеричный номер, который вычисляется из `uid` -
32-значного шестнадцатиричного идентификатора, используемого в паспорте.

- `get /pending_revision`: получение списка изделий на ревизии.

- `post /upload`: используется при выпуске паспорта. Выпускает паспорт, печатает qr-код, сохраняет данные в datalog
Робономики. С появлением модуля ОТК утратил актуальность.

- `post /assign-component/{unit_internal_id}`: добавить уже собранное изделие по его `unit_internal_id` как компонент
текущего изделия.
...
Статус изделия нельзя изменить напрямую запросом на ендпоинт, статус меняется во время процесса сборки.
...
- `get /status`: получение информации о статусе рабочего стола: статус, сотрудник, изделие, биография, итд.
- `post /assign-unit/{unit_internal_id}`: добавить изделие на сборку на данное РМИ. Подробнее про `unit_internal_id` в
[`unit`](#unit).
- `post /remove-unit`: удалить юнит с рабочего стола.
- `post /start-operation`: начать запись процесса сборки юнита.
- `post /end-operation`: закончить запись процесса сборки юнита.
- `get /production-schemas/names`: получить все схемы продуктов.
- `get /production-schemas/{schema_id}`: получить информацию о схеме по ее id.
- `post /hid-event`: обработка информацици с [HID-ридера](./feecc-hid-reader-daemon.md) и вызов функций на [хабе](./feecc-io-gateway.md) согласно hid-ивентам.
...
| Значение               | Описание |
| ---------------------- |          |
| AwaitLogin             | Начальное состояние РМИ. Ждет авторизации инженера посредством RFID. При логине происходит валидация данных и провродится проверка того, что такой сотрудник существует. |
| AuthorizedIdling       | Состояние РМИ после авторизации инженера. Простаивает до логаута сотрудника или помещения юнита на стол (посредством скана штрих-кодов). |
