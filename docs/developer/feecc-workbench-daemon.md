...
###### `post` /upload
> Используется при выпуске паспорта. Выпускает паспорт, печатает qr-код, сохраняет данные в datalog
...
###### `post` /hid-event
> Обработка информацици с [HID-ридера](./feecc-hid-reader-daemon.md) и вызов функций на [хабе](./feecc-io-gateway.md) согласно hid-ивентам.
...
| Функция | Описание |
| ------- | -------- |
| `Camera.py` | Осуществляет проверку наличия камеры и отправку запросов на [хаб](./feecc-io-gateway.md) на начало и остановку записи процесса сборки. |
| `Employee.py` | Осуществляет хранение данных о сотруднике в захешированном виде. |
| `IO_gateway.py` | Осуществляет коммуникацию с [хабом](./feecc-io-gateway.md) на предмет запросов на печать qr-кодов, запросов на загрузку файлов в IPFS, отправку данных в [datalog](https://wiki.robonomics.network/docs/en/rio-datalog/). |
...
| `passport_generation.py` | Формирование паспорта для дальнейшей загрузки в IPFS. |
